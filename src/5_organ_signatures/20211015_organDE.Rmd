...
test_de_ct <- function(subset_sce, test_cts, ctrl_cts, contrast_str, 
  ...
  subset_sce$anno_lvl_2_final_clean <- factor(subset_sce$anno_lvl_2_final_clean, levels=rev(cts))
  my_design = ~ 1 + donor + organ + anno_lvl_2_final_clean + anno_lvl_2_final_clean:organ 
...
organ_test <- "BM"
organ_ctrl <- "LI"
test_cts <- c('PRO_B', "PRE_PRO_B", "LATE_PRO_B", "SMALL_PRE_B")
organ_ref <- "SP"

subset_sce <- de_prep_data(sce, 
                           cts=test_cts,
                           organ_test = organ_test, organ_ctrl = organ_ctrl, organ_ref = organ_ref, n_hvgs = 7500
                           )


ctrl_cts <- unique(subset_sce$anno_lvl_2_final_clean[subset_sce$is_ctrl_ct])
contrast_str <- "`organLI:anno_lvl_2_final_clean{ct}` - `organBM:anno_lvl_2_final_clean{ct}`"

## Exclude genes not expressed in tested organs
# (testing on these gives a lot of pvals==1)
non_empty_rows_ls <- sapply(c(test_cts, ctrl_cts), function(ct){
  subset_sce$test_anno <- subset_sce$anno_lvl_2_final_clean == ct
  subset_sce$test_organs <- subset_sce$organ %in% c(organ_test, organ_ctrl)
  non_empty_rows <- which(rowSums2(counts(subset_sce[,subset_sce$test_anno & subset_sce$test_organs])) > 0)
  non_empty_rows
})

non_empty_rows <- Reduce(intersect, non_empty_rows_ls)
ct_subset_sce <- subset_sce[non_empty_rows,]

de_res_ls <- test_de_ct(ct_subset_sce, test_cts, ctrl_cts, contrast_str = contrast_str, organ_ref = organ_ref)
...
```{r}
plot_test_QC <- function(de_res){
  p1 <- ggplot(de_res, aes(pval)) + geom_histogram()
  p2 <- ggplot(de_res, aes(lfc, -log10(adj_pval))) +
    geom_point()
  p1 + p2
}

lapply(de_res_ls, plot_test_QC)
```
...
library(UpSetR)
names(de_res_ls) <- c(test_cts, ctrl_cts)[0:6]

# setNames(c(test_cts, ctrl_cts)[0:6], de_res_ls)
...
organ_test <- "TH"
organ_ctrl <- NULL
test_cts <- c("CD4+T", "CD8+T", "TREG", "NK_T", "TH17")

subset_sce <- de_prep_data(sce, 
                           cts=test_cts,
                           organ_test = organ_test, organ_ctrl = organ_ctrl, 
                           n_hvgs = 7500
                           )

ctrl_cts <- unique(subset_sce$anno_lvl_2_final_clean[subset_sce$is_ctrl_ct])

contrast_str <- "(`organTH` + `organTH:anno_lvl_2_final_clean{ct}`)"

## Exclude genes not expressed in tested celltypes
# (testing on these gives a lot of pvals==1)
non_empty_rows_ls <- sapply(c(test_cts, ctrl_cts), function(ct){
  subset_sce$test_anno <- subset_sce$anno_lvl_2_final_clean == ct
  subset_sce$test_organs <- subset_sce$organ %in% c(organ_test, organ_ctrl)
...
de_res_ls <- test_de_ct(ct_subset_sce, test_cts, ctrl_cts, contrast_str = contrast_str)
...
```
```{r}
names(de_res_ls) <- c(test_cts, ctrl_cts)[1:3]
...
```{r, echo=TRUE}
contrast_str <- "`organBM` + `organBM:anno_lvl_2_final_clean{ct}`"
...
my_design = ~ donor + organ + test_anno:organ + 1
my_contrast <- "(`organTH` + `organTH:test_annoTRUE`)"
