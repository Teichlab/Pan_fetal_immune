...
sce <- sce[,sce$anno_lvl_2 %in% keep_ct]
...
## Exclude high mito cells
sce <- sce[,sce$anno_lvl_2!="HIGH MITO CELLS"]
...
anno_groups <- split(colnames(sce), sce$anno_lvl_2)
all_hvgs <- c()
...
<!--   mutate(organ=sce$organ, celltype=sce$anno_lvl_2) %>% -->
...
<!-- ```{r} -->
<!-- sce_matureB <- sce[,sce$anno_lvl_2=="MATURE B CELL"] -->
...
<!--   mutate(organ=sce_matureB$organ, celltype=sce_matureB$anno_lvl_2) %>% -->
...
<!-- ``` -->
<!-- ```{r} -->
<!-- sce_matureB <- sce[,sce$anno_lvl_2=="NK"] -->
<!-- assay(sce_matureB, "scaled_logcounts") <- t(scale(t(logcounts(sce_matureB)))) -->
...
<!--          celltype=sce_matureB$anno_lvl_2) %>% -->
...
mofa <- create_mofa_from_SingleCellExperiment(sce[all_hvgs,], assay = "corrected_logcounts", 
                                              groups = "anno_lvl_2", extract_metadata = TRUE)
...
```
```{r, fig.width=18}
plot_data_heatmap(mofa_trained, factor = 2, show_colnames=FALSE, annotation_samples = c("anno_lvl_2", "organ"))
